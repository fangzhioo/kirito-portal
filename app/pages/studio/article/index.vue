    ...
    <div class="mt-5">
      <template>
        <v-card flat>
          ...
          <v-data-table
            no-data-text="No articles available, please publish article."
            :headers="headers"
            :items="videos"
            :search="search"
            :loading="loading"
          ...
          >
            <template v-slot:top>
              <v-dialog v-model="dialogDelete" persistent max-width="500px">
                <v-card>
                  <v-card-title>
                    <span class="headline">Permanently delete this article?</span>
                  </v-card-title>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn color="blue darken-1" text @click="deleteItem">Delete Forever</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </template>
...
export default {
  ...
  methods: {
    editItem(item) {
      this.$router.push({ name: `studio-articles-publish-id`, params: { id: item.id } });
    },
    deleteBtn(item) {
